...
export default ThemeContext.use(h => {
	const DatePicker = ({value, min, max, theme, ...props}, cleanup, mounted) => {
		...
		const getRow = date => {
			...
			let Div = <raw:div />;
			let content = <Div theme="date_row">
			...
			if (past.getDate() === 1) {
				...
				Div = <raw:div />;
				content = [
					<Div theme="date_sep">{month[past.getMonth()] + ' ' + year}</Div>,
					content
				];
			}
		...
		return <div {...props} theme="date_base">
			<div theme="date_header">
				{Array(7).fill(null).map((_, i) => {
					return <div theme="date_elem">{"SMTWTFS".charAt(i)}</div>;
				})}
			</div>
			<BodyComp />
		</div>;
